...
TitleLabel.Text = "get cracked :3 - astral"
...
local CORNERRRRR = Instance.new("UICorner")
CORNERRRRR.CornerRadius = UDim.new(0, 12)
CORNERRRRR.Parent = ESPButton
...
ESPButton.MouseButton1Click:Connect(function()
	...
	local function highestpartt(model)
	...
	local function esp(part, lines)
		local billboard = Instance.new("BillboardGui")
		billboard.Name = "brainrotesp"
		billboard.AlwaysOnTop = true
		billboard.Size = UDim2.new(0, 200, 0, #lines * 20)
			...
			for _, plot in ipairs(workspace.Plots:GetChildren()) do
				...
				if podiums then
					for _, podium in ipairs(podiums:GetChildren()) do
						local overhead = podium:FindFirstChild("Base") and podium.Base:FindFirstChild("Spawn") and podium.Base.Spawn:FindFirstChild("Attachment") and podium.Base.Spawn.Attachment:FindFirstChild("AnimalOverhead")
						if overhead then
							...
							local rarity = rarityLabel and rarityLabel:IsA("TextLabel") and rarityLabel.Text or ""
							local price = priceLabel and priceLabel:IsA("TextLabel") and priceLabel.Text or ""
							local displayName = nameLabel and nameLabel:IsA("TextLabel") and nameLabel.Text or ""
							local mutation = mutationLabel and mutationLabel:IsA("TextLabel") and mutationLabel.Text or ""

							local isSecret = string.find(string.lower(rarity), "secret") or string.find(string.lower(displayName), "secret")
							local isTarget = getgenv().ESP.TargetPets[displayName]

							if isSecret or isTarget then
								local model = podium:FindFirstChildWhichIsA("Model")
								if model then
									local topPart = highestpartt(model)
									if topPart then
										local distance = (LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")) and (topPart.Position - LocalPlayer.Character.HumanoidRootPart.Position).Magnitude or 0
										local meters = math.floor(distance + 0.5) .. "m"

										local lines = {
											displayName ~= "" and displayName or nil,
											rarity ~= "" and rarity or nil,
											price ~= "" and price or nil,
											mutation ~= "" and mutation or nil,
											meters
										}

										local vvvv = {}
										for _, v in ipairs(lines) do
											if v then table.insert(vvvv, v) end
										end
